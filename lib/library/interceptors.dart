...
class AuthInterceptor extends Interceptor {
  ...
  void onRequest(RequestOptions options, RequestInterceptorHandler handler) {
    if (StoreUtils.store.hasData(Constant.userKey)) {
      options.headers['Authorization'] =
          'Bearer ' + StoreUtils.store.read(Constant.userKey)['token'];
    } else {
      DioUtils.instance.requestNetwork(Method.post, LOGIN_PATH, params: {
        "phone": "15896272531",
        "password": "111111",
        'terminal': 'WEB'
      }).then((value) {
        StoreUtils.store.write(Constant.userKey, json.decode(value));
      });
    }
